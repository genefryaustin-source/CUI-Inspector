...
def render_manifest_export():
    st.header("📦 Evidence Export Manifest")
    tid = tenant_id()
    ...
    if not st.button("Generate manifest ZIP", type="primary"):
        return
    with db() as con:
        rows = con.execute("SELECT ef.id AS evidence_id,ef.inspection_id,ef.kind,ef.filename,ef.sha256,ef.size_bytes,ef.object_relpath,ef.created_at,i.started_at AS inspection_started_at,i.risk_level FROM evidence_files ef JOIN inspections i ON i.id=ef.inspection_id WHERE ef.tenant_id=? ORDER BY ef.created_at DESC", (tid,)).fetchall()
