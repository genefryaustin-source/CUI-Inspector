...
def render_manifest_export():
    ...
    with db() as con:
        rows = con.execute(
            ...
            LEFT JOIN artifact_versions av
                ON av.artifact_id = a.id AND av.tenant_id = a.tenant_id
            LEFT JOIN inspections i
                ON i.artifact_version_id = av.id AND i.tenant_id = a.tenant_id
            LEFT JOIN evidence_files ef
                ON ef.inspection_id = i.id AND ef.tenant_id = a.tenant_id
            WHERE a.tenant_id = ?
            ORDER BY
                a.logical_name,
                av.version_int DESC,
    ...
    for _, r in df.iterrows():
        if isinstance(r.get('artifact_sha256'), str) and isinstance(r.get('artifact_object_relpath'), str):
            hash_lines.append(
                f"{r['artifact_sha256']}  ARTIFACT  {r.get('original_filename','')}  {r['artifact_object_relpath']}"
            )
        if isinstance(r.get('evidence_sha256'), str) and isinstance(r.get('evidence_object_relpath'), str):
            hash_lines.append(
                f"{r['evidence_sha256']}  EVIDENCE:{r.get('evidence_kind','')}  {r.get('evidence_filename','')}  {r['evidence_object_relpath']}"
            )
