...
def render_compare():
    ...
    init_db()
    with db() as con:
        rows = con.execute("SELECT id,started_at,risk_level,patterns_json FROM inspections WHERE tenant_id=? ORDER BY started_at DESC LIMIT 500", (tid,)).fetchall()
    ...
    pa = json.loads(a["patterns_json"] or "{}"); pb = json.loads(b["patterns_json"] or "{}")
    keys = sorted(set(pa)|set(pb))
    out=[{"pattern":k,"older":int(pa.get(k,0)),"newer":int(pb.get(k,0)),"delta":int(pb.get(k,0))-int(pa.get(k,0))} for k in keys]
    st.subheader("Pattern deltas"); st.dataframe(out, use_container_width=True)
    st.subheader("Risk delta"); st.write({"older_risk": a["risk_level"], "newer_risk": b["risk_level"]})
