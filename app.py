...
# Optional OCR dependencies (Streamlit Cloud safe)
OCR_AVAILABLE = True
try:
...
try:
    ...
    Image = None
    # PIL is only used for OCR flows; keep OCR_AVAILABLE as-is
    ...
    def extract_text_from_pdf(self, file) -> str:
        ...
        if is_scanned:
            if not OCR_AVAILABLE:
                st.warning("⚠️ OCR dependencies not available in this deployment (pytesseract/pdf2image). Skipping OCR.")
                # Return whatever text we may have extracted; if none, raise a clear error.
                if not text.strip():
                    raise Exception("PDF appears scanned but OCR is unavailable. Install pytesseract + pdf2image and provide Poppler/Tesseract binaries.")
                return text
            try:
                # Reset file pointer
                file.seek(0)
...
st.caption(f"OCR available: **{OCR_AVAILABLE}**")
